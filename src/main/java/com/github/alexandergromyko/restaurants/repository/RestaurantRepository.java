...
public interface RestaurantRepository extends BaseRepository<Restaurant> {
    ...
    @Query("SELECT r FROM Restaurant r JOIN FETCH r.dishes d WHERE r.id=:id AND d.date=:date AND r.enabled ORDER BY r.name ASC, d.name ASC")
    List<Restaurant> getAllEnabledByIdWithDishes(int id, LocalDate date);
